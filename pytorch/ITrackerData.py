...
DATASET_PATH = 'D:/GazeCaputreData'
MEAN_PATH = './'
META_PATH = './meta_50x50.mat'
...
class ITrackerData(data.Dataset):
    def __init__(self, split = 'train', imSize=(224,224), gridSize=(50, 50)):
    ...
    def __getitem__(self, index):
        ...
        leftEyeGrid = self.makeEyeGrid(self.metadata['labelLeftEyeGrid'][index,:])
        rightEyeGrid = self.makeEyeGrid(self.metadata['labelRightEyeGrid'][index, :])

        # to tensor
        row = torch.LongTensor([int(index)])
        leftEyeGrid = torch.FloatTensor(leftEyeGrid)
        rightEyeGrid = torch.FloatTensor(rightEyeGrid)
        gaze = torch.FloatTensor(gaze)

        return row, imEyeL, imEyeR, leftEyeGrid, rightEyeGrid, gaze
