...
doTest = False #True # Only run test, no training

workers = 1
epochs = 100
batch_size = 64 #torch.cuda.device_count()*100# Change if out of cuda memory
...
def train(train_loader, model, criterion,optimizer, epoch):
    ...
    for i, (row, imEyeL, imEyeR, leftEyeGrid, rightEyeGrid, gaze) in enumerate(train_loader):
...
def validate(val_loader, model, criterion, epoch):
    ...
    oIndex = 0
    for i, (row, imEyeL, imEyeR, leftEyeGrid, rightEyeGrid, gaze) in enumerate(val_loader):
        # measure data loading time
        data_time.update(time.time() - end)
        imEyeL = imEyeL.cuda(async=True)
        imEyeR = imEyeR.cuda(async=True)
        ...
        losses.update(loss.data[0], imEyeL.size(0))
        lossesLin.update(lossLin.data[0], imEyeL.size(0))
